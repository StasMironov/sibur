...
======== UPLAB =======
DEV-30545
К исходному скрипту добавлены изменения
добавлено
masksChange.call(this,e);
в функции masksKeyDown
======== UPLAB =======
...
(function ($) {
	...
	var unbindOriginal = function () {
		...
		$.each(types, function (idx, evt_name) {
			$.each(
				that.inputmask.events[evt_name],
				function (evt_idx, evt_func) {
	...
	var maskApply = function (match, newtext) {
		...
		if (
			match &&
			(newtext !== undefined ||
				match.mask != this.inputmasks.oldmatch.mask)
		) {
			var caretPos;
